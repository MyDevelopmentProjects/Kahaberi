...
public abstract class PaginationAndFullSearchQuery<T> {
    ...
    public PaginationAndFullSearchQueryResult<T> getPaginatedList(String searchExpression,
                                                                  String sortField,
                                                                  boolean isAscending,
                                                                  int pageNumber,
                                                                  int pageSize,
        ...
        if (pageSize > 0 && (pageSize < 7 || pageSize > 100)) {
            pageSize = 10;
        }
        ...
        Predicate pdqExpression = getPDQExpression(clazz, objects);
        CriteriaBuilder criteriaBuilder = em.getCriteriaBuilder();
        CriteriaQuery<T> criteriaQuery = criteriaBuilder.createQuery(clazz);
        Root<T> returnClassRoot = criteriaQuery.from(clazz);
        returnClassRoot.alias(TABLE_ALIAS);


        if (searchExpression != null && searchExpression.length() > 0 && getFieldsAvailableForFullTextSearch(clazz) != null) {
        ...
        PaginationAndFullSearchQueryResult<T> result = new PaginationAndFullSearchQueryResult<>();

        CriteriaQuery<Long> countQuery = createCountQuery(criteriaQuery, clazz);
        if (countQuery != null) {
